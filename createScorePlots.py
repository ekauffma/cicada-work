...
def main(nBins):
	...
	cicada_names = ["CICADA_v1p2p0","CICADA_v2p2p0","CICADA_v1p2p0N","CICADA_v2p2p0N"]
	...
	for i in range(len(cicada_names)):
		...
		output_file = ROOT.TFile(f'hists_240220_{cicada_names[i]}.root', 'RECREATE')

		# make hists for zerobias
		histModel = ROOT.RDF.TH1DModel(f"anomalyScore_ZeroBias_{cicada_names[i]}", f"anomalyScore_ZeroBias_{cicada_names[i]}", nBins, min_score, max_score)
		hist = zero_bias.Histo1D(histModel, f"{cicada_names[i]}_score")
		hist.Write()

		eff = ROOT.TH1D(f"efficiency_ZeroBias_{cicada_names[i]}", f"efficiency_ZeroBias_{cicada_names[i]}", hist.GetNbinsX(), hist.GetXaxis().GetXmin(), hist.GetXaxis().GetXmax())
		...
		for k in range(len(sample_names)):				
			...
			eff = ROOT.TH1D(f"efficiency_{sample_names[k]}_{cicada_names[i]}",
